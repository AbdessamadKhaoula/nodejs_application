pipeline {
    ...
    stages {
	 ...
	 stage("Trigger CD Pipeline") {
             steps {
                 script {
                      sh "curl -v -k --user clouduser:${JENKINS_API_TOKEN} -X POST -H 'cache-control: no-cache' -H 'content-type: application/x-www-form-urlencoded' --data 'IMAGE_TAG=${IMAGE_TAG}' 'ec2-13-51-251-40.eu-north-1.compute.amazonaws.com:8080/job/NodeJs-app-CD/buildWithParameters?token=gitops-token'"
                  }
             }
         }
     }
