pipeline {
    ...
    stages {
	 ...
	 stage("Trigger CD Pipeline") {
             steps {
                 script {
                      sh "curl -v -k --user clouduser:${JENKINS_API_TOKEN} -X POST -H 'cache-control: no-cache' -H 'content-type: application/x-www-form-urlencoded' --data 'IMAGE_TAG=${IMAGE_TAG}' 'ec2-16-16-169-170.eu-north-1.compute.amazonaws.com/job/NodeJs-app-CD/buildWithParameters?token=gitops-token'"
                  }
             }
         }
     }
